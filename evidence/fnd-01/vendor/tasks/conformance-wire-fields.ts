...
import { errMsg, failureCheck, SEP_2663_REF } from "./mrtr-helpers";
  ...
  async run(ctx: RunContext): Promise<ConformanceCheck[]> {
    ...
    let createdTaskId: string | undefined;
    {
      ...
      try {
        ...
        // be absent.
        if (!("ttlMs" in result)) {
          errs.push("CreateTaskResult MUST carry ttlMs (renamed from v1 `ttl`)");
        } else if (
          result.ttlMs !== null &&
          (typeof result.ttlMs !== "number" || !Number.isInteger(result.ttlMs) || result.ttlMs <= 0)
        ) {
          errs.push(
        ...
        if ("ttl" in result) {
          errs.push("CreateTaskResult MUST NOT carry the v1 `ttl` key (use ttlMs)");
        }
    ...
    // Check 2: task accessible before TTL elapses.
    {
      ...
      const description = "Task remains accessible via tasks/get for the duration of its ttlMs";
      if (!createdTaskId) {
        checks.push(
          untestableCheck(
            id,
        ...
        try {
          ...
          if (after.taskId !== createdTaskId) {
            errs.push(`task MUST still be accessible before TTL; got taskId=${after.taskId}`);
          }
          checks.push({
            id,
            name,
      ...
      try {
        ...
        if (!taskId) {
          ...
          if (meta && meta["io.modelcontextprotocol/related-task"]) {
            errs.push("related-task _meta MUST NOT appear on tasks/get inlined result");
          }
          checks.push({
            id,
            name,
            ...
            details: {
              ...
              hasRelatedTask: Boolean(meta?.["io.modelcontextprotocol/related-task"]),
            },
          });
        }
      } catch (error) {
