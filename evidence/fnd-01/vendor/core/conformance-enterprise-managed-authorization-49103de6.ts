...
export class EnterpriseManagedAuthorizationScenario implements Scenario {
  ...
  async start(ctx: ScenarioContext): Promise<ScenarioUrls> {
    ...
    const authApp = createAuthServer(ctx, this.checks, this.authServer.getUrl, {
      ...
      tokenVerifier,
      onTokenRequest: async ({ grantType, body, timestamp, authBaseUrl, authorizationHeader }) => {
    ...
    const mcpApp = createServer(ctx, this.checks, this.mcpServer.getUrl, this.authServer.getUrl, {
      tokenVerifier,
    });
  ...
  private async startIdpServer(): Promise<void> {
    ...
    app.post("/token", async (req: Request, res: Response) => {
      ...
      try {
        ...
        const { payload } = await jose.jwtVerify(subjectToken, this.idpPublicKey!, {
  ...
  private async handleJwtBearerGrant(
    ...
    if (!authorizationHeader || !authorizationHeader.startsWith("Basic ")) {
      ...
      return {
        ...
        errorDescription: "Client authentication required (client_secret_basic)",
        statusCode: 401,
      };
    }
    ...
    const authClientSecret = decodeURIComponent(decoded.slice(separatorIndex + 1));
    ...
    try {
      ...
      if (jagClientId !== authClientId) {
        ...
        return {
          error: "invalid_grant",
          errorDescription: "ID-JAG client_id does not match authenticating client",
        };
      }
  ...
  getChecks(): ConformanceCheck[] {
    const hasTokenExchangeCheck = this.checks.some((c) => c.id === "complete-flow-token-exchange");
    const hasJwtBearerCheck = this.checks.some((c) => c.id === "complete-flow-jwt-bearer");
