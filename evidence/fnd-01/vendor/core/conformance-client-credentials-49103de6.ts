...
import type { ConformanceCheck, Scenario, ScenarioUrls } from "../../../types";
...
export class ClientCredentialsJwtScenario implements Scenario {
  ...
  async start(ctx: ScenarioContext): Promise<ScenarioUrls> {
      ...
      onTokenRequest: async ({ grantType, body, timestamp, authBaseUrl }) => {
        ...
        try {
          ...
          if (payload.iss !== CONFORMANCE_TEST_CLIENT_ID) {
            ...
            return {
              error: "invalid_client",
              errorDescription: "JWT iss claim does not match expected client_id",
              statusCode: 401,
            };
          }
          ...
          if (payload.sub !== CONFORMANCE_TEST_CLIENT_ID) {
            ...
            return {
              error: "invalid_client",
              errorDescription: "JWT sub claim does not match expected client_id",
              statusCode: 401,
            };
          }
          ...
          this.checks.push({
            ...
            description: "Client successfully authenticated with signed JWT assertion",
            status: "SUCCESS",
            timestamp,
            specReferences: [
              SpecReferences.OAUTH_2_1_CLIENT_CREDENTIALS,
    ...
    const app = createServer(ctx, this.checks, this.server.getUrl, this.authServer.getUrl);
  ...
  getChecks(): ConformanceCheck[] {
    ...
    const hasJwtCheck = this.checks.some((c) => c.id === "client-credentials-jwt-verified");
    if (!hasJwtCheck) {
      this.checks.push({
        id: "client-credentials-jwt-verified",
        name: "ClientCredentialsJwtVerified",
...
export class ClientCredentialsBasicScenario implements Scenario {
  ...
  async start(ctx: ScenarioContext): Promise<ScenarioUrls> {
    ...
    const authApp = createAuthServer(ctx, this.checks, this.authServer.getUrl, {
      ...
      onTokenRequest: async ({ grantType, body, timestamp, authorizationHeader }) => {
        if (grantType !== "client_credentials") {
          this.checks.push({
            id: "client-credentials-grant-type",
            name: "ClientCredentialsGrantType",
        ...
        if (!authHeader || !authHeader.startsWith("Basic ")) {
          this.checks.push({
            ...
            description: "Missing or invalid Authorization header for Basic auth",
            status: "FAILURE",
            timestamp,
        ...
        this.checks.push({
          id: "client-credentials-basic-auth",
          name: "ClientCredentialsBasicAuth",
          description: "Client successfully authenticated with client_secret_basic",
          status: "SUCCESS",
          timestamp,
          specReferences: [
            SpecReferences.OAUTH_2_1_CLIENT_CREDENTIALS,
  ...
  getChecks(): ConformanceCheck[] {
    ...
    const hasBasicAuthCheck = this.checks.some((c) => c.id === "client-credentials-basic-auth");
