...
| **Output schema** | ✅ | ✅ | Tool output schema in macros, handler.rs. Live `bind_http` and live shipped-echo stdio `structured_echo` retain `outputSchema` on `tools/list` and matching `structuredContent` on modern `tools/call`; a peer without an output schema stays bare. Live exact-2024 stdio lists the same `outputSchema` and still returns the handler text without inventing `structuredContent` |
...
| `resources/subscribe` | ✅ | ✅ | Session dispatch serves subscribe for registered URIs; registering a resource or template advertises `resources.subscribe` on initialize. Live exact-2024 HTTP+SSE `resources/subscribe` then `touch` delivers `notifications/resources/updated`; omitting only subscribe stays silent |
...
10. ✅ **Per-handler timeout** - Handler-level configuration exists and both modern and exact-2024 session dispatch now enforce it through `run_handler_in_request`. Live `bind_http` refuses a late tool and still admits a fast peer. Live shipped-echo stdio `slow_echo` is refused with `Request timeout exceeded` on both ModernOnly and LegacyOnly, then `fast_echo` still completes; panic-boundary hardening remains unit-proven
