...
| HTTP multi-client isolation | ✅ | 🟡 | The unsafe shared-Session listener is quarantined and unreachable. Live `bind_http` two independent ModernOnly clients each invoke the same tool with distinct arguments and both handler results are retained. Live public `bind_websocket` two independent ModernOnly sockets do the same (`tool:alpha` / `tool:beta`) without mixing results. Live exact-2024 HTTP+SSE two independent `legacy_2024::HttpClient` sessions do the same (`tool:alpha` / `tool:beta`) without mixing results. Aggregate request-execution qualification remains unverified |
