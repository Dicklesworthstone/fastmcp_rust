...
| **Duplicate handling** | ✅ | ✅ | `on_duplicate` in builder.rs. Live `bind_http` `DuplicateBehavior::Error` keeps the first `tools/call` handler (`tool:alpha`); changing only that flag to `Replace` installs the second handler (`replaced:alpha`) |
