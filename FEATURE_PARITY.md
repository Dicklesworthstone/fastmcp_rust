...
| **Icons support** | ✅ | ✅ | Icon metadata in types.rs, handler.rs. Live `bind_http` `tools/list` retains `final_icons` and `final_title` on the advertised tool; a near-identical peer without those hooks stays untitled and iconless |
...
| **Timeout per handler** | ✅ | ✅ | Handler timeout surface exists. Live `bind_http` a 10ms tool timeout refuses a late `tools/call` and still admits a near-identical fast peer tool. Live exact-2024 HTTP+SSE refuses the same late tool through `legacy_2024::HttpClient` and still admits the fast peer. Panic-boundary hardening remains unit-proven, not a live hang-tool e2e |
...
| Feature | Python | Rust | Notes |
|---------|--------|------|-------|
| **Resource reading from handler** | ✅ | ✅ | `ctx.read_resource()` in context.rs. Live `bind_http` compose reads the peer resource through the request-owned reader from a tool handler, a prompt handler, and a resource handler. Live shipped-echo stdio `compose_echo`, `compose_greeting`, and `info://compose` read `info://server` after the nested `echo` on both ModernOnly and LegacyOnly |
| **Tool calling from handler** | ✅ | ✅ | `ctx.call_tool()` in context.rs. Live `bind_http` compose calls the peer tool through the request-owned caller from a tool handler, a prompt handler, and a resource handler. Live shipped-echo stdio `compose_echo`, `compose_greeting`, and `info://compose` nest `echo` + `info://server` on both ModernOnly and LegacyOnly |
