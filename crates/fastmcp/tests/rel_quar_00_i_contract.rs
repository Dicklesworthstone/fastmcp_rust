...
fn rel_quar_00_i_planted_negative() {
    ...
    // ARM 1 -- the REL-02 receipt appears. One variable.
    ...
    let refusal = quarantine_release_surface(&inventory, &planted)
        .expect_err("a present REL-02 receipt must be refused");
    assert_eq!(refusal.code, "E_REL_02_PRESENT");

    // ARM 2 -- one zero-authority counter becomes non-zero. One variable.
    let mut planted = pristine.clone();
    planted.counters.registry_requests += 1;
    let refusal = quarantine_release_surface(&inventory, &planted)
        .expect_err("a non-zero registry-request counter must be refused");
    assert_eq!(refusal.code, "E_COUNTER_NON_ZERO");
    assert_eq!(refusal.field, "registry_requests");

    // ARM 3 -- the record is bound to a different A digest. One variable, and
    // it uses a digest already present in the record rather than inventing
    // one, so the mutation cannot fail for a malformed-value reason.
    let mut planted = pristine.clone();
    planted.a_digest = pristine.cells[0].state_digest;
    let refusal = quarantine_release_surface(&inventory, &planted)
        .expect_err("a record bound to a foreign A digest must be refused");
    assert!(
        refusal.code == "E_SLICE_BINDING" || refusal.code == "E_B_SLICE",
        "rebinding must be refused by the integration binding check or by B itself, \
         not accepted: got {}",
        refusal.code
    );

    // The pristine record is unchanged and still acceptable, byte for byte in
    // its accepted digest. Without this a refusal above could be an artefact
    // of a mutation that leaked into the shared input.
    let reaccepted = quarantine_release_surface(&inventory, &pristine)
        .expect("the pristine closure is still accepted after three planted arms");
    assert_eq!(
        reaccepted, accepted,
...
fn rel_quar_00_integration_planted_negative() {
    ...
    // One variable, through the EVALUATOR rather than the entrypoint, so this
    // pair's refusal path is exercised on its own subject.
    let mut planted = pristine.clone();
    planted.counters.provider_grants += 1;
    let refusal = rel_quar_00_integration_quarantine_closure(&inventory, &planted)
        .expect_err("a non-zero provider-grant counter must be refused");
    assert_eq!(refusal.code, "E_COUNTER_NON_ZERO");
    assert_eq!(refusal.field, "provider_grants");
