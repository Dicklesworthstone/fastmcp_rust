...
enum Commands {
    ...
    /// Inspect legacy background-task RPCs (quarantined by FastMCP servers).
    ///
    /// These commands target the pre-2026 custom task model for diagnostic
    /// compatibility only. They do not implement or claim MCP 2026-07-28 task
    /// conformance, and FastMCP servers reject these RPCs as method-not-found.
    ///
...
#[derive(Subcommand)]
enum TasksAction {
    /// Probe the legacy task-list RPC and display one bounded page.
    ///
    /// Example: fastmcp tasks list ./my-server -- --config config.json
    List {
        /// Server command or path.
    ...
    /// Probe the legacy task-detail RPC for one identifier.
    ///
    ...
    /// Probe the legacy task-cancellation RPC.
    ///
    ...
    /// Summarize one bounded page from the legacy task-list RPC.
    ///
...
fn task_status_json(status: TaskStatus) -> &'static str {
    match status {
...
fn bounded_task_info_value(
    task: &fastmcp_protocol::TaskInfo,
    budget: &mut JsonPreviewBudget,
) -> serde_json::Value {
    let mut output = serde_json::Map::new();
    output.insert(
        "id".to_owned(),
        bounded_json_string(task.id.as_str(), budget),
    );
    output.insert(
        "taskType".to_owned(),
        bounded_json_string(&task.task_type, budget),
    );
    output.insert(
        "status".to_owned(),
        serde_json::Value::String(task_status_json(task.status).to_owned()),
    );
    if let Some(progress) = task.progress
        && let Some(progress) = serde_json::Number::from_f64(progress)
    {
        output.insert("progress".to_owned(), serde_json::Value::Number(progress));
    }
    for (key, value) in [
        ("message", task.message.as_deref()),
        ("createdAt", Some(task.created_at.as_str())),
        ("startedAt", task.started_at.as_deref()),
        ("completedAt", task.completed_at.as_deref()),
...
fn bounded_task_result_value(
    result: &fastmcp_protocol::TaskResult,
    budget: &mut JsonPreviewBudget,
) -> serde_json::Value {
    let mut output = serde_json::Map::new();
    output.insert(
        "id".to_owned(),
        bounded_json_string(result.id.as_str(), budget),
    );
    output.insert(
        "success".to_owned(),
        serde_json::Value::Bool(result.success),
    ...
    if let Some(data) = &result.data {
        output.insert(
            "data".to_owned(),
            bounded_json_preview_inner(data, 0, budget),
        );
    }
    if let Some(error) = &result.error {
        output.insert("error".to_owned(), bounded_json_string(error, budget));
    }
    serde_json::Value::Object(output)
}

fn bounded_task_list_value(
    tasks: &[fastmcp_protocol::TaskInfo],
    ...
    let values = tasks
        .iter()
        .take(CLI_OUTPUT_MAX_ITEMS)
        .map(|task| bounded_task_info_value(task, &mut budget))
        .collect::<Vec<_>>();
    let has_more = tasks.len() > values.len() || next_cursor.is_some();
    budget.mutation.truncated |= has_more;
    serde_json::json!({
        "tasks": values,
        "hasMore": has_more,
...
fn cmd_tasks_list(
    ...
    let result = client.list_tasks(status_filter, None, Some(limit))?;
    let tasks = result.tasks;
    let next_cursor = result.next_cursor;

    if json_output {
        let envelope = bounded_task_list_value(&tasks, next_cursor.as_deref());
        let output = serde_json::to_string_pretty(&envelope).map_err(|e| {
            fastmcp_core::McpError::internal_error(format!("JSON serialization error: {e}"))
        })?;
        write_stdout(&output, "task list output", true)?;
        return Ok(());
    }

    if tasks.is_empty() {
        let message = if next_cursor.is_some() {
            "No tasks were returned in this bounded page. The peer reported additional tasks, but this command does not follow task cursors."
        } else {
            "No tasks found."
        };
        write_stdout(message, "task list output", true)?;
        return Ok(());
    }

    let mut output = String::new();
    let header = "ID | Type | Status | Progress | Created | Message";
    let _ = push_output_line(&mut output, "Legacy Background-Task RPC Results");
    let _ = push_output_line(&mut output, header);
    let _ = push_output_line(&mut output, &"-".repeat(header.len()));
    let mut rendered = 0usize;
    for task in tasks.iter().take(CLI_OUTPUT_MAX_ITEMS) {
        ...
        let id = sanitize_peer_text(task.id.as_str(), PEER_FIELD_LIMIT);
        let task_type = sanitize_peer_text(&task.task_type, PEER_FIELD_LIMIT);
        let message = task.message.as_deref().map_or_else(
            || "-".to_owned(),
            |value| sanitize_peer_text(value, PEER_DETAIL_LIMIT),
        );
        if !push_output_line(
            &mut output,
            &format!("{id} | {task_type} | {status_str} | {progress} | {created} | {message}"),
        ) {
            break;
        }
        rendered += 1;
    ...
    let omitted = tasks.len().saturating_sub(rendered);
    if omitted > 0 {
        let _ = push_output_line(&mut output, &format!("...[{omitted} tasks omitted]"));
    }

    if next_cursor.is_some() {
        let _ = push_output_line(
            &mut output,
            "\n(The peer reported additional tasks; this command displays one bounded page and does not follow task cursors.)",
        );
    }

    write_stdout(&output, "task list output", false)?;

    Ok(())
}

fn task_limit(limit: usize) -> McpResult<u32> {
    if limit == 0 {
        return Err(fastmcp_core::McpError::invalid_params(
            "Task limit must be at least 1",
        ));
    }
    if limit > CLI_OUTPUT_MAX_ITEMS {
        return Err(fastmcp_core::McpError::invalid_params(format!(
            "Task limit {limit} exceeds the CLI bounded-page maximum of {CLI_OUTPUT_MAX_ITEMS}"
        )));
    }
    u32::try_from(limit).map_err(|_| {
        fastmcp_core::McpError::invalid_params(format!(
            "Task limit {limit} cannot be represented by the protocol"
        ))
    })
}
...
fn cmd_tasks_show(client: &mut Client, task_id: &str, json_output: bool) -> McpResult<()> {
    ...
    if json_output {
        ...
        value.insert(
            "task".to_owned(),
            bounded_task_info_value(&result.task, &mut budget),
        );
        if let Some(result) = &result.result {
            value.insert(
                "result".to_owned(),
                bounded_task_result_value(result, &mut budget),
            );
        }
        value.insert(
            "redacted".to_owned(),
            serde_json::Value::Bool(budget.mutation.redacted),
        );
        value.insert(
            "sanitized".to_owned(),
            serde_json::Value::Bool(budget.mutation.sanitized),
        );
        value.insert(
            "truncated".to_owned(),
            serde_json::Value::Bool(budget.mutation.truncated),
        );
        let output =
            serde_json::to_string_pretty(&serde_json::Value::Object(value)).map_err(|e| {
                fastmcp_core::McpError::internal_error(format!("JSON serialization error: {e}"))
            })?;
        write_stdout(&output, "task detail output", true)?;
        return Ok(());
    }
    ...
    let _ = push_output_line(
        ...
        &format!(
            "Task: {}",
            sanitize_peer_text(task.id.as_str(), PEER_FIELD_LIMIT)
        ),
    );
    let _ = push_output_line(
        &mut output,
        &format!(
            "Type: {}",
            sanitize_peer_text(&task.task_type, PEER_FIELD_LIMIT)
        ),
    );
    let _ = push_output_line(
        &mut output,
    ...
    if let Some(progress) = task.progress {
        let _ = push_output_line(&mut output, &format!("Progress: {:.0}%", progress * 100.0));
    }
    ...
    if let Some(message) = &task.message {
        let _ = push_output_line(
            ...
            &format!(
                "Message: {}",
                sanitize_peer_text(message, PEER_DETAIL_LIMIT)
            ),
        );
    }
    ...
    let _ = push_output_line(
        &mut output,
        &format!("Created: {}", format_timestamp(&task.created_at)),
    );

    if let Some(started) = &task.started_at {
        let _ = push_output_line(
            &mut output,
            &format!("Started: {}", format_timestamp(started)),
        );
    }
    ...
    if let Some(completed) = &task.completed_at {
        let _ = push_output_line(
            &mut output,
            &format!("Completed: {}", format_timestamp(completed)),
        );
    }
    ...
    if let Some(error) = &task.error {
        let _ = push_output_line(
            &mut output,
            &format!("Error: {}", sanitize_peer_text(error, PEER_DETAIL_LIMIT)),
        );
    }

    if let Some(task_result) = &result.result {
        let _ = push_output_line(&mut output, "\nResult:");
        let _ = push_output_line(&mut output, &format!("  Success: {}", task_result.success));
        if let Some(data) = &task_result.data {
            if let Ok(pretty) = serde_json::to_string_pretty(&bounded_json_preview(data)) {
                for line in pretty.lines() {
                    if !push_output_line(&mut output, &format!("  {line}")) {
                        break;
                    }
                }
            }
        ...
        if let Some(err) = &task_result.error {
            let _ = push_output_line(
                &mut output,
                &format!("  Error: {}", sanitize_peer_text(err, PEER_DETAIL_LIMIT)),
            );
        }
    }

    write_stdout(&output, "task detail output", false)?;
...
fn cmd_tasks_cancel(client: &mut Client, task_id: &str, reason: Option<&str>) -> McpResult<()> {
    ...
    write_stdout(&output, "task cancellation output", true)?;

    Ok(())
}

/// Summarize one bounded diagnostic page from the legacy task-list RPC.
fn cmd_tasks_stats(client: &mut Client, json_output: bool) -> McpResult<()> {
    let page_limit = task_limit(CLI_OUTPUT_MAX_ITEMS)?;
    let page = client.list_tasks(None, None, Some(page_limit))?;
    let peer_has_more = page.next_cursor.is_some();
    let locally_truncated = page.tasks.len() > CLI_OUTPUT_MAX_ITEMS;
    let tasks = page
        .tasks
        .into_iter()
        .take(CLI_OUTPUT_MAX_ITEMS)
        .collect::<Vec<_>>();
    let truncated = peer_has_more || locally_truncated;
    ...
    if json_output {
        ...
        write_stdout(&output, "task statistics output", true)?;
        return Ok(());
    }

    let scope = if truncated {
        "Bounded legacy task page (additional tasks were not counted)"
    } else {
        "Complete legacy task page"
    };
    let output = format!(
        "Legacy Task Diagnostic Statistics\n=================================\nScope:     {scope}\nTotal:     {total}\nActive:    {active}\n  Pending:   {pending}\n  Running:   {running}\nCompleted: {completed}\nFailed:    {failed}\nCancelled: {cancelled}"
    );
    write_stdout(&output, "task statistics output", true)?;
...
fn format_timestamp(ts: &str) -> String {
    ...
    if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(ts) {
        ...
        sanitize_peer_text(ts, PEER_FIELD_LIMIT)
    }
}
...
mod tests {
    ...
    mod helper_functions {
        ...
        fn task_list_json_uses_an_explicit_pagination_envelope() {
            ...
            let value = bounded_task_list_value(&[task], Some("cursor-2"));

            assert_eq!(value["tasks"].as_array().map(Vec::len), Some(1));
        ...
        #[test]
        fn task_list_json_reports_peer_text_mutations() {
            let task = fastmcp_protocol::TaskInfo {
                id: fastmcp_protocol::TaskId::from_string("task-1"),
                task_type: "token=TASK_TYPE_SECRET".to_owned(),
                status: TaskStatus::Pending,
                progress: None,
                message: Some("line\nmessage".to_owned()),
                created_at: "2026-08-01T00:00:00Z".to_owned(),
            ...
            let value = bounded_task_list_value(&[task], None);
    ...
    mod integration {
        ...
        #[test]
        fn task_limit_rejects_values_above_the_cli_page_bound() {
            assert!(task_limit(0).is_err());
            assert_eq!(task_limit(20).unwrap(), 20);
            assert_eq!(
                task_limit(CLI_OUTPUT_MAX_ITEMS).unwrap(),
                u32::try_from(CLI_OUTPUT_MAX_ITEMS).unwrap()
            );
            assert!(task_limit(CLI_OUTPUT_MAX_ITEMS + 1).is_err());
            assert!(task_limit(usize::MAX).is_err());
        }
