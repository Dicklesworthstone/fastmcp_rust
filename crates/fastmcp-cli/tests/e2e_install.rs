...
impl Drop for ChildGuard {
    fn drop(&mut self) {
        ...
        if self.child.is_some() {
            eprintln!(
                "fastmcp install-test harness retained an unreaped child; aborting after cleanup failure"
            );
            std::process::abort();
        }
    }
...
fn e2e_install_semantic_noop_preserves_noncanonical_bytes_without_backup() {
    ...
    let path = claude_path(&home);
    create_secure_fixture_directory(&home, path.parent().unwrap());
    let original = br#"{"mcpServers":{"my-server":{"env":{"FASTMCP_PROTOCOL_POLICY":"auto"},"command":"/bin/echo"}},"metadata":{"format":"keep-me"}}"#;
    write_secure_config_fixture(&path, original);
...
fn e2e_read_and_semantic_noop_accept_xattrs_but_replacement_fails_closed() {
    ...
    let path = claude_path(&home);
    create_secure_fixture_directory(&home, path.parent().unwrap());
    let original = br#"{"mcpServers":{"my-server":{"env":{"FASTMCP_PROTOCOL_POLICY":"auto"},"command":"/bin/echo"}}}"#;
    write_secure_config_fixture(&path, original);
