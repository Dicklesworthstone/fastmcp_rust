...
impl Drop for ProcessGroupGuard {
    fn drop(&mut self) {
        ...
        if self.child.is_some() {
            eprintln!(
                "fastmcp test-command harness retained an unreaped child; aborting after cleanup failure"
            );
            std::process::abort();
        }
    }
...
"#;
    ...
    for (marker, description) in [
        ...
        loop {
            match read_linux_process_state_and_group(descendant_pid) {
                Ok(Some((state, _))) => {
...
"#;
    ...
    let deadline = Instant::now() + PROCESS_CLEANUP_DEADLINE;
    loop {
        match read_linux_process_state_and_group(target_pid) {
            Ok(Some((state, _))) => {
                if matches!(state, 'Z' | 'X' | 'x') {
                    break;
                }
