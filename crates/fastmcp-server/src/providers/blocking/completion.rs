...
impl<H: CompletionHandler + 'static> BlockingCompletion<H> {
    ...
    pub fn from_sync_hooks(handler: H, lane: BlockingHandlerLane) -> Self {
        Self { handler: Arc::new(handler), lane }
    }
}
...
impl<H: CompletionHandler + 'static> CompletionHandler for BlockingCompletion<H> {
    ...
    fn complete_legacy_async<'a>(
        &'a self, ctx: &'a McpContext, params: LegacyCompletionParams,
    ...
    fn complete_legacy_async_in_request<'a>(
        &'a self, ctx: &'a McpContext, cx: &'a Cx, params: LegacyCompletionParams,
    ) -> BoxFuture<'a, McpOutcome<CompletionValues>> {
        let handler = Arc::clone(&self.handler);
        Box::pin(async move {
            outcome(self.lane.execute(ctx, cx, move |ctx| handler.complete_legacy(ctx, params)).await)
        })
    }
    ...
    fn complete_final_async_in_request<'a>(
        ...
        let handler = Arc::clone(&self.handler);
        Box::pin(async move {
            outcome(self.lane.execute(ctx, cx, move |ctx| handler.complete_final(ctx, params)).await)
        })
    }
}
...
mod tests {
    ...
    struct Completer { calls: Calls, poller: std::thread::ThreadId }
    impl Completer {
    ...
    impl CompletionHandler for Completer {
        ...
        fn complete_legacy(&self, ctx: &McpContext, params: LegacyCompletionParams) -> McpResult<CompletionValues> {
            self.record(ctx, "legacy", serde_json::to_value(params).unwrap());
            Ok(serde_json::from_value(json!({"values":["legacy-value"],"total":1,"hasMore":false})).unwrap())
        }
        fn complete_final(&self, ctx: &McpContext, params: FinalCompletionParams) -> McpResult<FinalCompletionValues> {
            let cancel = params.argument.value == "cancel";
            self.record(ctx, "final", serde_json::to_value(params).unwrap());
            if cancel { ctx.request_cancellation().cancel(); }
            Ok(serde_json::from_value(json!({"values":["final-value"],"total":7,"hasMore":true})).unwrap())
        }
    }
    fn legacy_params() -> LegacyCompletionParams {
        serde_json::from_value(json!({"ref":{"type":"ref/prompt","name":"render"},
            "argument":{"name":"body","value":"alpha"},"_meta":{"progressToken":"p-97"}})).unwrap()
    }
    fn final_params() -> FinalCompletionParams {
        serde_json::from_value(json!({"ref":{"type":"ref/prompt","name":"render","title":"Exact title"},
            "argument":{"name":"body","value":"α\nβ"},
            "context":{"arguments":{"style":"formal","_meta":"ordinary context value"}},
            "_meta":{"com.example/context":"kept"}})).unwrap()
    }
    ...
    fn fixture(lane: BlockingHandlerLane) -> (BlockingCompletion<Completer>, Calls) {
        ...
        (BlockingCompletion::from_sync_hooks(Completer {
            calls: Arc::clone(&calls), poller: std::thread::current().id(),
        }, lane), calls)
    }
    ...
    fn completion_preserves_both_eras_and_all_async_entry_points_on_the_pool() {
        runtime(true).block_on(async {
            ...
            for result in [completion.complete_legacy_async(&ctx, legacy_params()).await,
                completion.complete_legacy_async_in_request(&ctx, ctx.cx(), legacy_params()).await] {
                let Outcome::Ok(result) = result else { panic!("legacy completion expected") };
                assert_eq!(serde_json::to_value(result).unwrap(), json!({"values":["legacy-value"],"total":1,"hasMore":false}));
            }
            for result in [completion.complete_final_async(&ctx, final_params()).await,
                completion.complete_final_async_in_request(&ctx, ctx.cx(), final_params()).await] {
                let Outcome::Ok(result) = result else { panic!("final completion expected") };
                assert_eq!(serde_json::to_value(result).unwrap(), json!({"values":["final-value"],"total":7,"hasMore":true}));
            }
            let calls = calls.lock().unwrap();
            assert_eq!(calls.len(), 4);
            assert_eq!(calls[0], ("legacy", serde_json::to_value(legacy_params()).unwrap()));
            assert_eq!(calls[1], calls[0]);
            assert_eq!(calls[2], ("final", serde_json::to_value(final_params()).unwrap()));
    ...
    fn completion_keeps_context_absence_empty_and_populated_states_distinct() {
        runtime(true).block_on(async {
            let ctx = McpContext::new(Cx::current().unwrap(), 97);
            let (completion, calls) = fixture(BlockingHandlerLane::new(1).unwrap());
            for context in [None, Some(json!({})), Some(json!({"arguments":{}})), Some(json!({"arguments":{"body":"x"}}))] {
                let mut wire = serde_json::to_value(final_params()).unwrap();
                wire.as_object_mut().unwrap().remove("context");
                if let Some(context) = context { wire["context"] = context; }
                let params: FinalCompletionParams = serde_json::from_value(wire.clone()).unwrap();
                assert!(matches!(completion.complete_final_async(&ctx, params).await, Outcome::Ok(_)));
    ...
    fn unavailable_pool_closed_lane_and_precancellation_never_invoke_completions() {
        for case in 0..3 {
            runtime(case != 0).block_on(async {
                ...
                assert!(matches!(completion.complete_legacy_async(&ctx, legacy_params()).await, Outcome::Err(_)));
                assert!(matches!(completion.complete_final_async(&ctx, final_params()).await, Outcome::Err(_)));
                assert!(calls.lock().unwrap().is_empty());
    ...
    fn completion_deadline_before_dispatch_and_cancellation_during_return_withhold_results() {
        runtime(true).block_on(async {
            ...
            assert!(matches!(completion.complete_final_async(&expired, final_params()).await, Outcome::Err(_)));
            assert!(calls.lock().unwrap().is_empty());
            let mut params = final_params();
            params.argument.value = "cancel".into();
            assert!(matches!(completion.complete_final_async(&ctx, params).await, Outcome::Err(_)));
            ...
            assert!(matches!(completion.complete_final_async(&peer, final_params()).await, Outcome::Ok(_)));
    ...
    fn abandoned_prompt_keeps_the_shared_resource_and_completion_lane_charged() {
        ...
        impl PromptHandler for HeldPrompt {
            fn definition(&self) -> Prompt {
                Prompt { name: "held".into(), description: None, arguments: vec![], icon: None, version: None, tags: vec![] }
            }
        ...
        impl ResourceHandler for ReadyResource {
            fn definition(&self) -> Resource {
                Resource { uri: "report://ready".into(), name: "Ready".into(), description: None,
                    mime_type: None, icon: None, version: None, tags: vec![] }
            }
            fn read(&self, _ctx: &McpContext) -> McpResult<Vec<ResourceContent>> {
                self.0.fetch_add(1, Ordering::SeqCst);
                Ok(vec![serde_json::from_value(json!({"uri":"report://ready","text":"ready"})).unwrap()])
            }
        }
        runtime(true).block_on(async {
            let ctx = McpContext::new(Cx::current().unwrap(), 97);
            ...
            let prompt = BlockingPrompt::from_sync_hooks(HeldPrompt {
                started: Mutex::new(Some(started)), release: Mutex::new(blocked),
            }, lane.clone()).unwrap();
            let observed = Arc::new(AtomicUsize::new(0));
            let resource = BlockingResource::from_sync_hooks(ReadyResource(Arc::clone(&observed)), lane.clone()).unwrap();
            let (completion, calls) = fixture(lane.clone());
            ...
            drop(call);
            assert_eq!(lane.in_flight().unwrap(), 1, "dropping the waiter is not worker completion");
            assert!(matches!(resource.read_async(&peer).await, Outcome::Err(_)));
            assert!(matches!(completion.complete_final_async(&peer, final_params()).await, Outcome::Err(_)));
            ...
            let drain = peer.clone().with_operation_deadline(Some(peer.cx().now().saturating_add_nanos(5_000_000_000)));
            lane.wait_idle(&drain).await.unwrap();
            let Outcome::Ok(contents) = resource.read_async(&peer).await else { panic!("resource must recover capacity") };
            assert_eq!(serde_json::to_value(contents).unwrap()[0]["text"], "ready");
            assert!(matches!(completion.complete_final_async(&peer, final_params()).await, Outcome::Ok(_)));
            assert_eq!(observed.load(Ordering::SeqCst), 1);
            assert_eq!(calls.lock().unwrap().len(), 1);
            lane.close().unwrap();
            assert!(matches!(completion.complete_legacy_async(&peer, legacy_params()).await, Outcome::Err(_)));
            assert_eq!(calls.lock().unwrap().len(), 1);
        });
    }
}
